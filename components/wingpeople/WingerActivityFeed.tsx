...
function StatCallout({ count, label }: { count: number; label: string }) {
  return (
    <View
      className="bg-primary-soft rounded-2xl px-4 py-3 mb-4 flex-row items-center gap-3"
      style={{ borderWidth: 1, borderColor: 'rgba(90,140,58,0.18)' }}
    >
      <Text className="font-serif text-primary" style={{ fontSize: 36, letterSpacing: -1 }}>
        {count}
      </Text>
      <Text className="text-sm text-primary font-medium flex-1" style={{ lineHeight: 18 }}>
        {label}
      </Text>
    </View>
  );
...
function PeopleTab() {
  ...
  const matchCount = data.filter((item) => item.status === 'matched').length;
  ...
  return (
    <View style={{ gap: 8 }}>
      <StatCallout
        count={matchCount}
        label={matchCount === 1 ? 'successful match' : 'successful matches'}
      />
...
function PhotosTab() {
  ...
  const acceptedCount = data.filter((item) => item.status === 'approved').length;
  ...
  return (
    <View style={{ gap: 8 }}>
      <StatCallout
        count={acceptedCount}
        label={acceptedCount === 1 ? 'photo accepted' : 'photos accepted'}
      />
...
function PromptsTab() {
  ...
  return (
    <View style={{ gap: 8 }}>
      <StatCallout
        count={acceptedCount}
        label={acceptedCount === 1 ? 'prompt response accepted' : 'prompt responses accepted'}
      />
...
export function WingerActivityFeed() {
