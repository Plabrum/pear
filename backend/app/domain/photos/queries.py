...
async def fetch_suggested_photos(db: AsyncSession, suggester_id: Sqid, limit: int) -> list[SuggestedPhotoRow]:
...
async def fetch_own_photos(db: AsyncSession, user_id: Sqid) -> list[PhotoRow]:
...
async def fetch_dating_profile_owner(db: AsyncSession, dating_profile_id: Sqid) -> Sqid | None:
...
async def fetch_dater_push_and_suggester_name(
    db: AsyncSession, dater_id: Sqid, suggester_id: Sqid
