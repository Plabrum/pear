...
class Profile(
    StateMachineMixin(state_enum=UserRole, initial_state=UserRole.DATER),
):
    __tablename__ = "profiles"

    # `state` (DATER|WINGER) is the user's current mode — the single source of truth,
    # added by StateMachineMixin and flipped only via the role transition actions.
    # "Winging" is just role == WINGER (a dater keeps their hidden dating_profile).
