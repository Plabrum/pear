"""Phase-4 RLS gate — exercises the app-managed Row-Level-Security policy set.

This is **the gate** described in `docs/migration/04-auth-and-rls.md` (§"Test
suites"). It proves that the policies in
`alembic/versions/..._rls_policies_and_functions_*.py` enforce Pear's
relationship-scoped authorization floor at the *database* layer — independent of
any application code.

How it works
------------
The `graph` fixture seeds a representative dater/winger/match slice on a session
connected as the NON-superuser `pear_app` role under `app.is_system_mode = true`,
so RLS is bypassed during seeding *via the honored system-mode escape* (NOT a
superuser connection — `pear_app` is fully subject to FORCE RLS). Each test then
re-enters the *same* savepoint transaction as a concrete actor via
`acting_as(<uuid>)`, which mirrors `app/utils/deps.py:provide_transaction`::

    SET LOCAL app.is_system_mode = false    -- drop the trusted-op escape
    SET LOCAL app.user_id = '<actor-uuid>'  -- read by public.current_user_id()

so every policy evaluates against `<actor>`. There is no `SET ROLE`: the
connection role *is* the non-superuser `pear_app` throughout. Visibility /
mutation are then asserted across the graph.

The contract under test is `app.user_id` (uuid) on the non-superuser `pear_app`
role; `public.current_user_id()` reads the GUC. An UNSET `app.user_id` (with the
escape off) must fail closed.

Matrix coverage (docs/migration/04 §"RLS")
------------------------------------------
- [row 1] Dater sees only own dating profile / matches / messages.
- [row 2] Winger reads their dater's discover-relevant rows + may insert decisions
          on the dater's behalf (active contact) — but NOT for unrelated daters.
- [row 3] Match participants (and only they) read + send messages in that match;
          a non-participant sees none.
- [row 4] Unapproved photos / prompt_responses visible to owner + suggester only;
          approved visible to authed viewers.
- [row 5] No `contacts` link -> only public profile fields (profiles SELECT true),
          nothing relationship-scoped.
- [row 6] `app.user_id` UNSET -> policies DENY (fail closed) on the RLS tables.

Run with::

    ENV=testing uv run pytest tests/test_rls.py
"""
