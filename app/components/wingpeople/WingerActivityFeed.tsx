...
function StatCallout({ count, label }: { count: number; label: string }) {
  return (
    <View className="flex-row items-baseline gap-1.5 px-4 pb-3">
      <Text className="font-serif text-primary" style={{ fontSize: 22, letterSpacing: -0.4 }}>
        {count}
      </Text>
...
function PeopleTab() {
  ...
  const matchedCount = data.filter((item) => item.status === 'matched').length;
  ...
  return (
    <>
      <StatCallout count={matchedCount} label={matchedCount === 1 ? 'match made' : 'matches made'} />
      <View style={{ gap: 8 }}>
...
function PhotosTab() {
  ...
  return (
    <>
      <StatCallout
        count={approvedCount}
        label={approvedCount === 1 ? 'photo approved' : 'photos approved'}
      />
      <View style={{ gap: 8 }}>
...
function PromptsTab() {
  ...
  return (
    <>
      <StatCallout
        count={acceptedCount}
        label={acceptedCount === 1 ? 'response accepted' : 'responses accepted'}
      />
      <View style={{ gap: 8 }}>
